...
app.get("/", async (req, res) => {
  try {
    const response = await axios.get(masterKey);
...
app.get("/jokes/:id", (req,res)=>{
  ...
  let jk = jokes.find((joke) => joke.id === id);
  res.json(jk);
});
//3. GET a jokes by filtering on the joke type
app.get("/filter", (req,res)=>{
  let type = req.query.type;
  let jkf = jokes.filter((jokef) => jokef.jokeType === type);
  res.json(jkf);
});
//4. POST a new joke
...
app.put("/jokes/:id", (req,res) => {
  ...
  let jkp = jokes.find((joke) => joke.id === ido);
  jkp = {
    id: ido,
   jokeText : req.body.text,
   jokeType : req.body.type,
...
app.delete("/jokes/:id", (req,res) => {
  ...
  let jkd = jokes.findIndex((joke) => joke.id === idd);
  if(jkd > -1){
    jokes.splice(jkd, 1);
    res.sendStatus(200);
  }else{
